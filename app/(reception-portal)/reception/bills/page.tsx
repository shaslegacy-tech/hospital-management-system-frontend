...
import { Receipt, IndianRupee, Plus, Search, X, ChevronLeft } from "lucide-react";
...
import { AppointmentResponse, BillResponse, PatientResponse } from "@/lib/types";
...
export default function ReceptionBillsPage() {
  ...
  return (
    <>
      <Topbar title="Bills" subtitle="Create and manage patient billing" profileHref="/reception/dashboard" />
      ...
      <div className="space-y-6 px-6 pb-10 lg:px-10">
        ...
        {loading ? (
          ...
          <EmptyState icon={Receipt} title="No bills yet" description="Create a bill after a completed visit." />
        ) : (
                ...
                >
                  ...
                  <span className="flex justify-end">
                    {b.status === "PENDING" ? (
                      ...
                      <span className="text-xs text-ink-500">{b.paymentMethod || "—"}</span>
                    )}
                  </span>
                </div>
              ))}
...
function CreateBillModal({
  ...
  const [step, setStep] = useState<"patient" | "appointment" | "details">("patient");
  ...
  return (
    <Modal open={open} onClose={onClose} title="Create bill">
      <div className="space-y-4">
        ...
        {step === "appointment" && selectedPatient && (
          <>
            ...
            <p className="text-sm text-ink-700">
              ...
              <span className="font-semibold">{selectedPatient.patientName}</span>
            </p>
            {loadingAppointments ? (
              <Skeleton className="h-16 w-full" />
            ) : appointments.length === 0 ? (
              <Alert tone="info">
                No completed appointments found for this patient yet.
              </Alert>
            ) : (
              <div className="space-y-2">
                {appointments.map((a) => (
                  <button
                    ...
                    className="flex w-full items-center justify-between rounded-xl border border-ink-100 p-2.5 text-left hover:border-brand-300 hover:bg-brand-50"
                  >
                    <div>
                      <p className="text-sm font-medium text-ink-900">{a.doctorName}</p>
                      <p className="text-xs text-ink-500">
                        {formatDate(a.appointmentDate)} · {a.departmentName}
                      </p>
                    </div>
        ...
        {step === "details" && selectedAppointment && selectedPatient && (
          <>
            <button
