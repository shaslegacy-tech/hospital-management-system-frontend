...
const staticNav: NavItem[] = [
  ...
  { href: "/reception/pending-approvals", label: "Pending Approvals", icon: Clock },
...
// ─── Hook — fetches live pending count ────────────────────
function useReceptionNav(): NavItem[] {
  ...
  return staticNav.map((item) =>
    item.href === "/reception/pending-approvals"
...
export function ReceptionSidebar() {
  ...
  // ✅ Use hook instead of static nav
  const nav = useReceptionNav();
  ...
  return (
    <aside className="hidden w-64 flex-shrink-0 flex-col border-r border-ink-100 bg-white px-4 py-6 lg:flex">
      ...
      <nav ref={navRef} className="relative mt-8 flex-1 space-y-1">
        ...
        {/* ✅ Now uses dynamic nav with badge */}
